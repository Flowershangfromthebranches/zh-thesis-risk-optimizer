...
- 黑灰冻结：黑色低风险文本和灰色不检测文本默认冻结，灰色包括过短片段、标题、英文、参考文献和学校模板等。
- 一步到位策略矩阵：当目标 AIGC 在 20% 以下、出现平台期或需要跨专业处理时，启用 `one_pass_cross_discipline_strategy`，按专业、风险率和颜色分布选择不同策略。
...
Level 4 不是全文模式，只能局部用于符合专业 profile 和 section profile 的高风险残留段。摘要、理论基础、结论禁止聊天化表达。紫色处理用于降低全文统计一致性，不用于大幅改写。若目标 AIGC 为 20% 以下且当前仍高于目标，紫色默认进入强制低强度重平衡；黑色和灰色默认冻结。
...
- 跨专业或希望“一步到位”时，使用 [references/one_pass_cross_discipline_strategy.md](references/one_pass_cross_discipline_strategy.md) 的策略矩阵：高 AIGC 优先红橙结构重建，中段风险处理橙紫联动，接近 20% 时只处理残留橙紫和模板痕迹，避免为了字数扩写造成回升。
